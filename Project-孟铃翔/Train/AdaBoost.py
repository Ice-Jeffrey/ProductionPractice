...
def model_with_negative():
    ...
    for i in range(iter):
        ...
        print("第{}个模型，准确率：{}\n预测结果：{}".format(i, accuracy, test))   

    # 将结果写入文件
    WritePredictions('Results/AdaBoostResultWithNegative.csv', accuracy_list, test_list)
...
def model_with_only_positive():
    ...
    return accuracy_score(predictions, y_test), test
...
def main():
    ...
    data['降维前预测值'] = list3
    data['降维后预测值'] = list4
    data.to_csv('Results/AdaBoostWithOnlyPositive.csv', index=False)
